...
| `gate` | `DiagnosticGate` — see §4. `null` for logcat/kernel captures (no check run), and for a debug capture whose run did not complete (the reason is in `errors`). Only a completed run reports `ROUTED`. In an agent-bridge read (`captureKind: agent_bridge`) the gate and `report` describe the latest complete measurement the app retains (`ROUTED`), else the blocking condition of the latest attempt that never started, else `null`; they come from the same presentation value as `diagnostics`, whose `applicability` says whether that measurement still applies. |
