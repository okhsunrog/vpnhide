...
3. **Turn on the Java layer.** In LSPosed / LSPosed-Next / Vector, enable
   **VPN Hide** and add **System Framework** to its scope. Without that scope the
   Java hooks don't attach.
4. **(Optional) Install Ports** if you need localhost port hiding.
5. **Reboot** so the native and Java modules load.
