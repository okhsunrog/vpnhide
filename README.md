...
2. **Один Native-бэкенд.** «Обзор» определит ядро и назовёт файл — kmod, KPM или Zygisk. Установите его через root-менеджер и перезагрузитесь. → [Какой Native-бэкенд выбрать](docs/help/ru/choosing-native.md) · [kmod](docs/help/ru/kmod-install.md) · [KPM](docs/help/ru/kpm-install.md) · [Zygisk](docs/help/ru/zygisk-install.md)
